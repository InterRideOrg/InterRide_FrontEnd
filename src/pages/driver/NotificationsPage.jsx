...
export default function DriverNotificationsPage() {
  ...
  useEffect(() => {
    const fetchDriverNotifications = async () => {
      try {
        ...
        const profileRes = await axiosProtected.get(`/usuario/profile/DriverId/${userId}`);
        const conductorId = profileRes.data;

        // Step 2: Fetch notifications with order=desc
        const notiRes = await axiosProtected.get(
          `/notificaciones/conductor/${conductorId}?orden=desc`
        );
  ...
  const getNotificationStyle = (mensaje) => {
    ...
    if (mensaje.includes('confirmado') || mensaje.includes('aceptado') || mensaje.includes('reserva')) {
      return { 
  ...
  return (
    <MainLayout title={isMobile ? "🚗 Notificaciones" : "🚗 Centro de Notificaciones de Conductor"}>
      <Container 
        maxWidth="md" 
        sx={{ 
          py: isMobile ? 2 : 4,
            ...
            {notifications.length === 0 && (
              ...
              >
                ...
                <Typography 
                  ...
                  color="text.secondary"
                >
                  Cuando tengas nuevas reservas o actualizaciones aparecerán aquí
                </Typography>
              </Paper>
            )}
          </Stack>
